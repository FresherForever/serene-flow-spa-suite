...
function Test-Endpoint {
    ...
    try {
        ...
        Write-Host "✅ OK ($($response.StatusCode)) - ${responseTimeMs}ms" -ForegroundColor $successColor
...
function Invoke-ServerHealing {
    ...
    Write-Host "`n[🔧] Attempting to heal $ServerType server..." -ForegroundColor $healingColor
    ...
    if ($ProcessInfo.IsRunning -and -not $IsResponding) {
        ...
        try {
            if ($ProcessInfo.ProcessId) {
                ...
                Write-Host "  ✅ Successfully stopped process (PID: $($ProcessInfo.ProcessId))" -ForegroundColor $successColor
            ...
            Write-Host "  ❌ Failed to restart process: $($_.Exception.Message)" -ForegroundColor $errorColor
    ...
    elseif (-not $ProcessInfo.IsRunning) {
        ...
        try {
            ...
            # Wait for server to start
            Write-Host "  Waiting for server to initialize (15 seconds)..." -ForegroundColor $infoColor
            Start-Sleep -Seconds 15        } catch {
            Write-Host "  ❌ Failed to start process: $($_.Exception.Message)" -ForegroundColor $errorColor
...
if ($attemptedHealing) {
    ...
    foreach ($action in $healingLog) {
        Write-Host "• $action" -ForegroundColor $infoColor
    }
}
...
if ($frontendResult.Success -and $backendResult.Success) {
    Write-Host "✅ All servers are running correctly!" -ForegroundColor $successColor
    ...
    Write-Host "❌ One or more servers are not running correctly." -ForegroundColor $errorColor
